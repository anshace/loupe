<!--
reviewer-v9 — single-pass PR reviewer prompt.

Changes from reviewer-v7 (the previous default):
  - adds {{RELATED_TESTS}} (report item #17): deterministically-discovered
    sibling test files for the changed sources, so the model can judge whether
    changed behavior is covered. Carries one FACTUAL coverage-gap rubric line
    (never an "add tests" nag).
  - adds {{CODE_HISTORY}} (report item #20): a compact git-blame summary of how
    old / how churny each changed region is, so the model leans toward
    pre-existing for old stable code and takes a fresh finding seriously for
    just-churned code. Read-only background, never grounds for a finding.
  (v8 remains the flagged variant; its successor reviewer-v10 also carries these
  two blocks. v7/v8 were left untouched per the never-edit-shipped rule.)
...
reviewer-v10.md (see prompts/README.md).
-->
