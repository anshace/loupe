<!--
reviewer-v8 — single-pass PR reviewer prompt with two OPTIONAL, flag-driven
placeholder blocks. Used only when the engine has a reason to (few-shot
exemplars and/or the walkthrough narrative); it renders IDENTICALLY to
reviewer-v7 when both placeholders are inert, so it is not a behavior change on
its own — the flags drive what fills the placeholders.

Changes from reviewer-v7:
  - adds {{FEWSHOT_EXEMPLARS}} (report item #14): 2–4 curated true/false-positive
    worked examples the engine injects when the `fewShotExemplars` flag is on,
    else "(none)". Few-shot exemplars are an UNPROVEN precision lever pending
    live-eval measurement; default OFF.
  - adds {{WALKTHROUGH_INSTRUCTION}} (report item #26): when the `walkthrough`
    flag is on, the engine injects an instruction to ALSO emit a short prose
    walkthrough by wrapping the output as {"walkthrough": "...", "findings": [...]}
    (the guardrail reads the findings array out of that wrapper, and fails open
    if the field is absent). When off, the injected text tells the model to keep
    the bare findings-array contract. Default OFF.
...
reviewer-v9.md (see prompts/README.md).
-->
