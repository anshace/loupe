<!--
reviewer-v13 — single-pass PR reviewer prompt: the flagged counterpart to
reviewer-v9, carrying every OPTIONAL, flag-driven placeholder block (few-shot
exemplars + walkthrough narrative + pre-flagged dangerous-sink evidence, as in
v11) PLUS two cheap read-only context blocks — a ranked repository map and a
ctags-lite symbol index. It carries the SAME {{RELATED_TESTS}} and
{{CODE_HISTORY}} context blocks v9 added. Selected only when the engine has a
reason to (few-shot exemplars, walkthrough, dangerous-sink pack, repo map, or
symbol index); it renders IDENTICALLY to reviewer-v9 when every flag placeholder
is inert.

Changes from reviewer-v11:
  - adds {{REPO_MAP}} (rounding-out item): a concise repository-structure sketch
    (top directories + key exported symbols from the changed files), ambient
    orientation only. "(none)" (and no behavior change) unless the repoMap flag
    is on.
  - adds {{SYMBOL_INDEX}} (rounding-out item, ctags-lite): where the symbols this
    PR touches are DECLARED across the repo (file:line), from a lightweight text
    scan. "(none)" (and no behavior change) unless the ctagsIndex flag is on.
    v11 was left untouched per the never-edit-shipped rule.
...
everything below it is the USER message template. The engine substitutes:
  ...
  {{REPO_MAP}}            — ranked repo structure + changed-file exports, or "(none)"
  {{SYMBOL_INDEX}}        — where touched symbols are declared (ctags-lite), or "(none)"
  {{TOOLS}}               — whether tool access is enabled for this run
Never edit this file after it has been used in a real run; changes go to
reviewer-v14.md (see prompts/README.md).
-->
...
The user message may include a "Repository map" section: a short structural
sketch of the repository (its top directories by file count) plus the key
exported symbols declared in the files this PR changes. It is READ-ONLY ambient
orientation — never grounds for a finding on its own, and never a line to anchor
to. When it is "(none)", ignore it.

## Symbol index (definitions)

The user message may include a "Symbol index" section: for the symbols this PR
touches, where each is DECLARED across the repository (`file:line`), from a
lightweight text scan. Use it to locate a definition you want to reason about —
but it is a heuristic index, not proof; confirm with the diff, the enclosing
context, or a tool before reporting, and never anchor a finding to a
symbol-index line. "(none)" means nothing resolved.
...
## Repository map (read-only structural sketch)

Top directories and the key exported symbols in the changed files. Ambient
orientation only — never grounds for a finding. "(none)" means it is disabled or
empty.
...
## Symbol index (heuristic definition locations)

Where the symbols this PR touches are declared across the repo (`file:line`),
from a text scan. Not proof — confirm before relying on it, and never anchor to
these lines. "(none)" means it is disabled or nothing resolved.
