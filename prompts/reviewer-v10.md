<!--
reviewer-v10 — single-pass PR reviewer prompt with two OPTIONAL, flag-driven
placeholder blocks (few-shot exemplars + walkthrough narrative). This is the
flagged counterpart to reviewer-v9: it carries the SAME {{RELATED_TESTS}} and
{{CODE_HISTORY}} context blocks v9 added, PLUS v8's {{FEWSHOT_EXEMPLARS}} and
{{WALKTHROUGH_INSTRUCTION}}. Selected only when the engine has a reason to
(few-shot exemplars and/or the walkthrough narrative); it renders IDENTICALLY to
reviewer-v9 when both flag placeholders are inert.

Changes from reviewer-v8:
  - adds {{RELATED_TESTS}} (report item #17) and {{CODE_HISTORY}} (report item
    #20), matching reviewer-v9. v8 was left untouched per the never-edit-shipped
    rule.
...
reviewer-v11.md (see prompts/README.md).
-->
