<!--
reviewer-v6 — single-pass PR reviewer prompt.

Changes from reviewer-v5:
  - adds the {{CROSS_FILE_CALLERS}} block (report item #8 — cross-file recall):
    when the diff changes an exported function/method signature, the engine
    deterministically finds its call sites in OTHER files (regex import graph)
    and force-injects them here, so the reviewer can check whether every caller
    was updated to match the new signature — the class of bug diff-only review
    is blind to;
  - documents the new `find_importers` agentic tool alongside grep / read_file.
...
reviewer-v7.md (see prompts/README.md).
-->
